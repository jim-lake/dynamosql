...
export function greatest(
  ...
  return _compare(expr, state, 'GREATEST', _gtList, (d) => d > 0);
}
export function least(
  expr: Function,
  state: EvaluationState
): EvaluationResult {
  return _compare(expr, state, 'LEAST', _ltList, (d) => d < 0);
}
...
type NativeCompareFunction = (delta: number | bigint) => boolean;
export function _compare(
  expr: Function,
  state: EvaluationState,
  functionName: string,
  dateCompare: typeof _ltList,
  nativeCompare: NativeCompareFunction
): EvaluationResult {
  ...
  const collation = getExprCollation(expr.args.value, state);
      ...
      case 'longlong':
        {
          ...
          for (const sub of values) {
            if (val === undefined) {
              val =
                typeof sub.value === 'bigint' ? sub.value : Number(sub.value);
            } else if (
              typeof val === 'bigint' &&
              typeof sub.value === 'bigint'
            ) {
              if (nativeCompare(sub.value - val)) {
                val = sub.value;
              }
            } else if (typeof val === 'bigint') {
              const val_big = BigInt(Number(sub.value));
              if (nativeCompare(val_big - val)) {
                val = val_big;
              }
            } else if (typeof sub.value === 'bigint') {
              val = BigInt(val);
              if (nativeCompare(sub.value - val)) {
                val = sub.value;
              }
      ...
      case 'string':
        {
          ...
          for (const sub of values) {
            ...
            if (
              val === undefined ||
              nativeCompare(stringCompare(val_s, val, collation))
            ) {
              val = val_s;
            }
          }
...
interface DateCompareListResult {
  value: EvaluationResult['value'];
  decimals: number;
}
type DateCompareFunc =
  | typeof convertDateTime
  | typeof convertDate
  | typeof convertTime;
function _dateCompareList(
  values: EvaluationResult[],
  type: ValueType,
  state: EvaluationState,
  dateCompare: typeof _ltList,
  convertFunc: DateCompareFunc
): DateCompareListResult {
  ...
  const ret = dateCompare(
    values,
    timeZone,
    (val, dec) => convertFunc({ value: val, timeZone, decimals: dec }) ?? null
  );
  if (ret === null) {
    value = null;
  } else {
    decimals = ret.decimals;
    if (ret.value && type === 'string' && typeof ret.value !== 'string') {
